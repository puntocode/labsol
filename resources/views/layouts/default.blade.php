...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" {{ Metronic::printAttrs('html') }} {{ Metronic::printClasses('html') }}>
    <head>
        ...
        @foreach(config('layouts.resources.css') as $style)
            <link href="{{ config('layouts.self.rtl') ? asset(Metronic::rtlCssPath($style)) : asset($style) }}" rel="stylesheet" type="text/css"/>
        @endforeach
    ...
    <body {{ Metronic::printAttrs('body') }} {{ Metronic::printClasses('body') }}>
        ...
        @foreach(config('layouts.resources.js') as $script)
